...
const Cards = (props) => {
    ...
    const checkId = () => {
        if (auth.currentUser.uid === props.value.idCard) {
    ...
    return (
        <section className="p-4 mt-8   md:w-[25rem]  h-full ">
            ...
            <div className="flex flex-col h-[39rem]  bg-white text-gray-800 items-center p-8 transition-colors duration-300 transform border rounded-xl  ">
                {showMenuDelete && (
                ...
                <img src={profil} alt="photo profil" className="rounded-full shadow-xl w-24 h-24 object-cover" />
                <h2 className="mt-4 text-2xl font-semibold  capitalize text-pink">{value.Prenom} {value.Nom}</h2>

                
                <div className="flex gap-2 text-4xl my-2 mx-2 text-gray-500">
                    {showLinkedin && (
                        <button>
                ...
                <p className="mt-2 text-sm p-2 flex flex-wrap bg-red-200 w-full "> {value.Description} "</p>

                <h4 className="my-4 font-bold">Je cherche 🔍</h4>
                <div className="flex flex-wrap justify-center">
                    <p className="ml-2 mt-1 bg-pink text-white py-1 px-2 rounded-lg">{value.WantCompetence}</p>
                </div>
